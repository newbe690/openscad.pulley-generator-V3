...
// retainer / idler now have 3 states:
//   0 = none
//   1 = raised flange (protruding outward)
//   2 = recess (groove) cut inward from the outer radius, BEFORE teeth are cut
// For value 2:
//   *_ht    = radial width of the groove (also height of raised flange for value 1)
//   *_width = axial depth of the groove
retainer = 0;			// Top retainer: 0 = none, 1 = raised flange, 2 = recess for separate retainer
retainer_ht = 1.5;		// flange height / radial width of recess, standard = 1.5
retainer_width = 1.5;	// axial depth of the recess (only for retainer == 2), standard = 1.5

idler = 0;				// Bottom idler: 0 = none, 1 = raised flange, 2 = recess for separate idler
idler_ht = 1.5;			// flange height / radial width of recess, standard = 1.5
idler_width = 1.5;		// axial depth of the recess (only for idler == 2), standard = 1.5
...
// bolt holes around shaft (through holes parallel to shaft axis)
bolt_count = 0;			// number of bolt holes, 0 = disabled
bolt_pcd = 15;			// pitch circle diameter of bolt holes (mm)
bolt_dia = 3.2;			// bolt hole diameter (3.2 for M3, 4.2 for M4), mm
bolt_angle_offset = 0;	// rotation offset of the first hole (degrees)

// bolt head recess on top side
// 0 = none (plain through hole)
// 1 = cylindrical recess for socket head cap screw (DIN 912)
// 2 = 45 deg countersink for countersunk screw (DIN 7991)
bolt_head_type = 0;
bolt_head_dia = 5.8;	// recess diameter (DIN912 M3 ~ 5.8; DIN7991 M3 ~ 6.2), mm
bolt_head_depth = 3.2;	// recess depth for type 1 (DIN912 M3 ~ 3.0 + clearance), mm; ignored for type 2
...
module pulley( belt_type , pulley_OD , tooth_depth , tooth_width )
	...
	// warn if retainer + idler grooves overlap axially
	if ( (retainer == 2 || idler == 2) && ((retainer == 2 ? retainer_width : 0) + (idler == 2 ? idler_width : 0) > pulley_t_ht) )
		echo("WARNING: retainer_width + idler_width exceed pulley_t_ht, grooves will overlap!");
	...
	difference()
	 {	 
		...
		difference()
			...
			// grooves for retainer/idler are cut HERE, before teeth,
			// so no tooth tips are left hanging in the air
			...
			difference()
				...
				// top groove (retainer == 2)
				// outer radius = pulley_OD/2 + 1 (clears tooth tips)
				// inner radius = pulley_OD/2 - retainer_ht + 1 (so real radial width = retainer_ht)
				if ( retainer == 2 )
				{
					translate([0,0, pulley_b_ht + pulley_t_ht])
					rotate_extrude($fn=teeth*4)
					polygon([
						[pulley_OD/2 - retainer_ht + 1, -retainer_width - 0.5],
						[pulley_OD/2 + 1,               -retainer_width - 0.5],
						[pulley_OD/2 + 1,                0.5],
						[pulley_OD/2 - retainer_ht + 1,  0.5],
						[pulley_OD/2 - retainer_ht + 1, -retainer_width - 0.5]
					]);
				}

				// bottom groove (idler == 2)
				// outer radius = pulley_OD/2 + 1 (clears tooth tips)
				// inner radius = pulley_OD/2 - idler_ht + 1 (so real radial width = idler_ht)
				if ( idler == 2 )
				{
					translate([0,0, pulley_b_ht])
					rotate_extrude($fn=teeth*4)
					polygon([
						[pulley_OD/2 - idler_ht + 1, -0.5],
						[pulley_OD/2 + 1,            -0.5],
						[pulley_OD/2 + 1,             idler_width + 0.5],
						[pulley_OD/2 - idler_ht + 1,  idler_width + 0.5],
						[pulley_OD/2 - idler_ht + 1, -0.5]
					]);
				}
				}
	
			//teeth - cut out of shaft (after grooves, so no hanging tips)
		...
		//belt retainer / idler - raised flange only when value == 1
		if ( retainer == 1 ) {translate ([0,0, pulley_b_ht + pulley_t_ht ]) 
		rotate_extrude($fn=teeth*4)  
		polygon([[0,0],[pulley_OD/2,0],[pulley_OD/2 + retainer_ht , retainer_ht],[0 , retainer_ht],[0,0]]);}
		
		if ( idler == 1 ) {translate ([0,0, pulley_b_ht - idler_ht ]) 
		rotate_extrude($fn=teeth*4)  
		...
		// bolt holes around shaft (parallel to shaft axis, through all)
		// and optional head recess (cylindrical or 45 deg countersink) on top
		if ( bolt_count > 0 )
		{
			for ( k = [0 : bolt_count-1] )
			rotate([0,0,bolt_angle_offset + k*(360/bolt_count)])
			translate([bolt_pcd/2, 0, 0])
			{
				// through hole
				translate([0,0,-1])
				cylinder(r=bolt_dia/2, h=pulley_b_ht + pulley_t_ht + retainer_ht + 2, $fn=bolt_dia*4);

				// head recess (top side)
				if ( bolt_head_type == 1 )
				{
					// cylindrical recess for socket head cap screw (DIN 912)
					translate([0,0, pulley_b_ht + pulley_t_ht + retainer_ht - bolt_head_depth])
					cylinder(r=bolt_head_dia/2, h=bolt_head_depth + 1, $fn=bolt_head_dia*4);
				}
				if ( bolt_head_type == 2 )
				{
					// 45 deg countersink for countersunk screw (DIN 7991)
					translate([0,0, pulley_b_ht + pulley_t_ht + retainer_ht - (bolt_head_dia - bolt_dia)/2])
					cylinder(r1=bolt_dia/2, r2=bolt_head_dia/2, h=(bolt_head_dia - bolt_dia)/2 + 1, $fn=bolt_head_dia*4);
				}
			}
		}
